...
fn split_filter_stats(
    n: u64,
    stats: Vec<(ReplicaId, StateMessage<SimpleEnclave>)>,
) -> Vec<Vec<StateMessage<SimpleEnclave>>> {
    let mut stats_by_replica: Vec<Vec<StateMessage<SimpleEnclave>>> =
...
fn check_errors(
    ...
    stats: &[Vec<StateMessage<SimpleEnclave>>],
) -> Option<String> {
    let mut result_string = String::new();
    for i in (0u64..n).filter(|e| !faultys.contains(&ReplicaId::from_u64(*e))) {
...
fn check_non_equivocation_broadcast(
    ...
    for i in 0u64..n {
        ...
        if faultys.contains(&ReplicaId::from_u64(i)) {
...
fn check_deduplication(
    ...
    for tx in delivery.into_iter().filter(|e| !faultys.contains(&e.0)) {
    ...
    for replica_count in d_counter.iter().filter(|e| !faultys.contains(e.0)) {
    ...
    let mut result_string = String::new();
    for replica_count in d_counter.into_iter().filter(|e| !faultys.contains(&e.0)) {
...
pub fn check(
    ...
    if let Some(e) = check_errors(n, faulty_nodes, &stats_by_replica) {
        return Some(format!("Errors !! {}", e));
    }
    if let Some(e) = check_peer_errors(n, faulty_nodes, &stats_by_replica) {
        return Some(format!("Peer Errors !! {}", e));
    }

    if faulty_nodes.is_empty() {
        // if let Some(e) = check_non_equivocation_broadcast(n, faulty_nodes, &stats_by_replica) {
        //     return Some(format!("Non Equivocation Broadcast !! {}", e));
        // }

        if let Some(e) = check_buffer(n, faulty_nodes, &stats_by_replica) {
            return Some(format!("Buffer !! {}", e));
        }

        if let Some(e) = check_graph(n, faulty_nodes, &stats_by_replica) {
            return Some(format!("Graph !! {}", e));
        }
    }

    if let Some(e) = check_round_transition(n, faulty_nodes, &stats_by_replica) {
        return Some(format!("Round Transition !! {}", e));
    }
    if let Some(e) = check_consensus(n, faulty_nodes, &stats_by_replica) {
